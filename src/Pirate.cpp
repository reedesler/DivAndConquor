...
void Pirate::update() {
    ...
    if (!path.path.empty()) {
        ...
        if (abs(position.x - destX) <= PIRATE_VELOCITY * 2 && abs(position.y - destY) <= PIRATE_VELOCITY * 2) {
...
void Pirate::travel(vec2 destination) {
    ...
    if (length > PIRATE_VELOCITY) {

        renderDirection(dir);

        dir = {dir.x / length, dir.y / length};

        addForce({dir.x * PIRATE_VELOCITY, dir.y * PIRATE_VELOCITY});
    }
}
