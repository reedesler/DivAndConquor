...
bool TriSprite::init(float width, float height, const char *textureName, vec2 texPiece)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(TexturedVertex) * 3, vertices, GL_STATIC_DRAW);
...
void TriSprite::draw(const mat3 &projection, vec2 position, float rotation, vec2 scale)
{
